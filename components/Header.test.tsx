...
function snapshot(branding: Record<string, unknown>, name = "SizeTube"): InstanceConfigSnapshot {
...
describe("Header branding", () => {
  ...
  it("renders the instance name from the snapshot while no logo is set", () => {
    ...
    expect(screen.getByRole("link", { name: "SizeTube" })).toBeTruthy();
    expect(screen.queryByRole("img")).toBeNull();
  });
  ...
  it("renders a set wide logo beside the name, resolved against the API origin", () => {
    ...
    expect(screen.getByText("SizeTube")).toBeTruthy();
  ...
  it("hide_instance_name drops the text only when a logo exists (alt carries the name)", () => {
    ...
    expect(screen.queryByText("SizeTube")).toBeNull();
    const imgs = Array.from(document.querySelectorAll("img"));
    expect(imgs.length).toBe(2);
    for (const img of imgs) expect(img.getAttribute("alt")).toBe("SizeTube");
  });
