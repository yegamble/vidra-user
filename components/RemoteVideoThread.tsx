...
//
// It is READ-ONLY: these rows are somebody else's thread and nothing here
// writes one. Local authoring lives ABOVE it, in RemoteVideoComments, whose
// composer writes to this instance and federates to the origin. Every mirrored
// comment carries its object_url on the origin so a reader can go there.
//
// The `comments` are handed in (RemoteVideoComments owns the single fetch that
// returns both the mirrored thread and the local `authored` set), so this is a
...
export function RemoteVideoThread({ comments }: { comments: RemoteVideoComment[] }) {
  const rows = threadComments(comments);
