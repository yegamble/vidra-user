...
export function useListQuery(options: UseListQueryOptions = {}): ListQuery {
  ...
  // Memoised on the key NAMES, not on the array's identity: `filterKeys` is
  // written as an inline literal at most call sites, which is a fresh array on
  // every render. Depending on that identity would make `filters` — and every
  // memo derived from it, including a fetch effect keyed on the list state — a
  // new value each render, i.e. an endless refetch loop.
  //
  // Joined on a separator no param name can contain, written as an escape: a
  // literal NUL in the source would make this file binary to git and to grep.
