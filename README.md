...
`PUBLIC_BASE_URL` per request in `middleware.ts`; the `E2E_*` vars in
...
| `PUBLIC_BASE_URL` | runtime, server only | The **site** origin — a different thing from the API origins above. Its scheme decides whether the middleware emits `Strict-Transport-Security`: https, unset, or unparseable emit (fail-secure); an explicit `http://` origin (the deliberate plain-http deployment mode) suppresses it |
