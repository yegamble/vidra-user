...
test(
  "a DM attachment is never mirrored to IPFS, on either swarm",
  // @ipfs keeps this out of the main backed matrix, which runs the whole
  // e2e-backed/ directory against a stack with no kubo node. See ipfs.spec.ts
  // for why this is a tag rather than a runtime `test.skip()` probe.
  { tag: "@ipfs" },
  async ({ page, request }) => {
