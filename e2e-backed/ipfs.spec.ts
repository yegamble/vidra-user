...
test(
  "a published video is pinned to IPFS, resolves through the real gateway, and the watch page plays from the mirror",
  // @ipfs marks this as needing the compose `ipfs` profile + IPFS_ENABLED. The
  // main backed matrix runs the WHOLE e2e-backed/ directory against a stack with
  // neither, so it excludes this tag (`--grep-invert @ipfs`); the dedicated
  // ipfs-backed job runs these two files by path.
  //
  // Deliberately a TAG rather than the runtime `test.skip()` feature probe
  // channel-sync.spec.ts uses. That idiom is right for a spec the general legs
  // still run, but the ipfs-backed job runs ONLY these two specs — so a
  // self-skip would let the one job that exists to exercise IPFS report green
  // having executed nothing. Excluding by tag can go quiet in a job that was
  // never meant to run it; it cannot go quiet in the job that was.
  { tag: "@ipfs" },
  async ({ page, request }) => {
