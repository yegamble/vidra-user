...
- [~] Implement channel block with subscribe button. (`ChannelView` now renders a `SubscribeButton` in the channel header: anonymous → a "Sign in to subscribe" link (mock-tested in `e2e/channel.spec.ts`); authenticated → a Subscribe/Subscribed toggle calling `api.followChannel`/`unfollowChannel` (`POST`/`DELETE /channels/:handle/follow`) with an optimistic follower-count nudge. The authed data-mutating flow is **NOT yet VERIFIED / BLOCKED**, but the reachability blocker is now RESOLVED: video cards link to `/channels/{handle}` (backend ships `channel_handle`; the card renders the link), so a logged-in session CAN reach a channel page from the feed. The remaining blocker is the **e2e auth rate-limit headroom**: a backed e2e for follow (signup + seed a channel/video + subscribe → follower_count↑) would push the backed suite past vidra-core's 10/min auth limit. Unblock = make the auth rate limit configurable/disabled in the e2e backend (tracked), then add the follow backed e2e.)
