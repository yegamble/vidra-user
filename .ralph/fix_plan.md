...
- [~] Add backend-backed e2e proving interactions that mutate data (comment, like, save, watch-progress) persist to the database and reappear after refetch. (comment + like + save DONE — `e2e-backed/comments.spec.ts`, `rating.spec.ts`, and `save.spec.ts` drive the UI and confirm the row via API/UI-refetch + psql. watch-progress pending its backend.)
