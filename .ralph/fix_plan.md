...
- [~] Implement manual block/unblock controls. (**Unblock DONE & VERIFIED**: each block-list card has an **Unblock** button → `DELETE /api/v1/admin/videos/:id/block` (`api.unblockVideo`), optimistically dropping the row; a fresh refetch keeps it gone and the video returns to public surfaces. The **block** action from the UI (e.g. a "Block video" control on the watch page / moderation report card) is DEFERRED to a follow-up slice — today a moderator blocks via the API contract; the block-list + unblock is the management surface. Mocked `e2e/blocked-videos.spec.ts` (anon gated, admin sees list via tabs, unblock removes row — 3). Backed `e2e-backed/blocked-videos.spec.ts` proves the unblock round trip.)
