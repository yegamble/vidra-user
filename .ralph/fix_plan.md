...
- [x] Implement account settings page. (`app/settings/page.tsx` → `components/auth/SettingsView.tsx`, auth-gated: shows a sign-in prompt when the in-memory session is gone (hard reload), else the profile form. Reached via the header username link (`AccountMenu` → `/settings`, client-side nav so the session survives).)
